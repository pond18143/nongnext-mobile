...
async function jMessage3(dataFBase, state) {
    ...
    if (state == 1) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            data.contents.contents[i].hero.aspectMode = "fit";
    ...
    // list product
        ...
        for (let i = 0; i < count; i++) {
            ...
            data.contents.contents[i].footer.contents[0].action.text = "add product " + dataFBase[i].id;//button action
        }
        var data = await JSON.stringify(data);
        return data;
    }
